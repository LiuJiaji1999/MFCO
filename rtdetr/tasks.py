...
class BaseModel(nn.Module):
    ...
    def forward(self, x, *args, **kwargs):
        ...
        # 如果输入是 dict，但用户要求提取特征 (layers=True)，就走 predict
        if isinstance(x, dict) and not layers:  # for cases of training and validating while training.
...
class RTDETRDetectionModel(DetectionModel):
    ...
    def loss(self, batch, preds=None):
        ...
        """
        # print('RTDETRDetectionModel/loss')
    ...
    def predict(self, x, profile=False, visualize=False, batch=None, augment=False,layers=False):
        """
        ...
        """
        # print('RTDETRDetectionModel/predict')
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, DSConv, Focus,
                 ...
                 C3_Star, C2f_Star, C3_Star_CAA, C2f_Star_CAA, C3_KAN, C2f_KAN, KANC3):
            if args[0] == 'head_channel':
                args[0] = d[args[0]]
            c1, c2 = ch[f], args[0]

    ...
    def _do_train(self, world_size=1):
        ...
        while True:
            ...
            for i, batch in pbar: # 等同于batch_size 大小
                ...
                # hyper-params (可在 trainer 初始化时设为属性)
                lambda_g = getattr(self, 'lambda_g', 1.0)
                lambda_l = getattr(self, 'lambda_l', 0.5)
                tau = getattr(self, 'tau', 0.07)  # contrastive temperature
                K = 3  # patch grid KxK
                use_roi = True  # 是否启用 RoI-based local loss
                roi_output_size = (3, 3)  # RoIAlign 输出大小
                eps = 1e-6
                ...
                with autocast(self.amp):
                    ...
                    # ---------- Global InfoNCE (pooled features) ----------
                    # Use highest layer(s) for global, e.g., layer 10
                    for l in [10]:
                        feas = self.backbone_feature_dict.get(l, None)
                        if feas is None:
                            continue
                        # feas: [(B*V), C, H, W]
                        pooled = F.adaptive_avg_pool2d(feas, 1).view(B, V, -1)  # [B, V, C]
                        # normalize
                        pooled_norm = F.normalize(pooled, dim=-1)  # [B, V, C]

                        # InfoNCE multi-positive formulation:
                        # for each sample b, construct logits: sim_matrix = (V x C) @ (V x C).T -> [B, V, V]
                        logits = torch.matmul(pooled_norm, pooled_norm.transpose(1,2)) / tau  # [B, V, V]
                        # mask self
                        diag_mask = torch.eye(V, device=device).bool().unsqueeze(0)
                        logits = logits.masked_fill(diag_mask, float('-inf'))

                        # For each anchor view i, positives are all other views j != i (multi-positive)
                        # Convert multi-positive to cross-entropy: for anchor i, target distribution proportional to sum_{pos} exp(logit)
                        # Implement using stable log-sum-exp:
                        # loss_bi = - log ( sum_pos exp(logit_i_pos) / sum_all exp(logit_i_all) )
                        exp_logits = torch.exp(logits)  # [B,V,V] (pos entries nonzero, self = 0)
                        sum_pos = exp_logits.sum(dim=2)  # sum over pos j for each i -> [B,V]
                        sum_all = sum_pos + 1e-10  # same because self exp = 0
                        # But negatives are other samples in batch: implement by expanding memory bank across B if desired.
                        # Simpler: treat per-sample negatives only (intra-sample). Then:
                        loss_global_b = -torch.log((sum_pos) / (sum_all + eps) + eps).mean()  # mean over B,V
                        # the above reduces to 0 because denom==num? To be robust, use alternative:
                        # use cross-entropy with labels where positive set index mask, but here approximate:
                        global_loss = global_loss + loss_global_b
